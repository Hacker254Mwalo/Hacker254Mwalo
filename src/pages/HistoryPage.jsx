...
import { getInvestments, getDeposits } from '../lib/db'
  ...
  return (
    <div className="pt-4 md:pt-20 pb-24 md:pb-8 px-4 max-w-2xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-gray-400 text-sm mt-1">Track your investments & deposits</p>
      </div>

      {/* Tab switcher */}
      <div className="flex gap-1 bg-gray-800 p-1 rounded-xl mb-6">
        {['investments', 'deposits'].map(t => (
          <button
            key={t}
            onClick={() => setTab(t)}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                        ...
                        {dep.status === 'approved' ? '✓ Approved' : dep.status === 'rejected' ? '✗ Rejected' : '⏳ Pending'}
                      </span>
                    </div>
                    <p className="font-semibold">M-Pesa Deposit</p>
                    <p className="text-gray-500 text-xs mt-0.5">
