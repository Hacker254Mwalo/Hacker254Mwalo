  ...
  return (
    <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="card max-w-sm w-full" onClick={e => e.stopPropagation()}>
        ...
        {/* 2B: Withdrawal step pipeline */}
        <div className="flex items-center gap-1 mb-4">
          {['Method', 'Amount', 'Confirm', 'Sent'].map((label, i) => (
            <div key={i} className="flex items-center gap-1 flex-1">
              <div className="flex flex-col items-center flex-1">
                <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[8px] font-black mb-0.5 transition-all ${
                  step > i ? 'bg-green-500 text-white' : step === i ? 'bg-blue-500 text-white scale-110' : 'bg-gray-700 text-gray-500'
                }`}>
                  {step > i ? '✓' : i + 1}
                </div>
                <span className={`text-[7px] font-bold uppercase ${step === i ? 'text-blue-400' : step > i ? 'text-green-400' : 'text-gray-600'}`}>{label}</span>
              </div>
              {i < 3 && <div className="h-px flex-1 mb-3" style={{ background: step > i ? '#22c55e' : 'rgba(255,255,255,0.08)' }} />}
            </div>
          ))}
        </div>
      ...
      <div className="card max-w-sm w-full" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex gap-2 mb-4">
          <a
            ...
            className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl font-black text-sm text-white transition-all hover:scale-[1.02] active:scale-95"
          ...
          <a href={`sms:?body=${encodeURIComponent(shareText)}`} className="btn-secondary px-4 text-xs">SMS</a>
        </div>
